...
public class PointsToAnalysis extends ForwardFlowAnalysis<Unit, PointsToGraph> {
    ...
    public boolean mayAlias(String leftVariableName, String rightVariableName) {
        ...
        Set<Node> set1 = getLastPointsToGraph().getNodesForVariable(leftVariableName);
        Set<Node> set2 = getLastPointsToGraph().getNodesForVariable(rightVariableName);

        for(Node nodo : set1){
            if(set2.contains(nodo)) return true;
        }
        return false;
    }
    ...
    public boolean mayAlias(String leftVariableName, String fieldName, String rightVariableName) {
        ...
        // throw new UnsupportedOperationException("Not implemented yet");
        Set<Node> set1 = getLastPointsToGraph().getNodesForVariable(leftVariableName);
        Set<Node> alcanzablesPorFieldname = new HashSet<>();

        for(Node nodo : set1){
            alcanzablesPorFieldname.addAll(getLastPointsToGraph().getReachableNodesByField(nodo, fieldName));
        }
        Set<Node> set2 = getLastPointsToGraph().getNodesForVariable(rightVariableName);

        for(Node nodo : alcanzablesPorFieldname){
            if(set2.contains(nodo)) return true;
        }
        return false;
    }
