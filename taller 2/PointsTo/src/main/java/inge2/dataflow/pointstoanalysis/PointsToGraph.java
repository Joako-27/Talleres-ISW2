...
public class PointsToGraph {
    ...
    public void addEdge(Node leftNode, String fieldName, Node rightNode) {
         // TODO: IMPLEMENTAR
         //throw new UnsupportedOperationException("Not implemented yet");
        Axis nuevo = new Axis(leftNode, fieldName, rightNode);
        this.axis.add(nuevo);
    }
    ...
    public Set<Node> getReachableNodesByField(Node node, String fieldName) {
        // TODO: IMPLEMENTAR
         //throw new UnsupportedOperationException("Not implemented yet");
        Set<Node> resultados = new HashSet<>();
        for(Axis eje : this.axis){
            if(eje.leftNode.equals(node) && eje.fieldName.equals(fieldName)) resultados.add(eje.rightNode);
        }
        return resultados;
    }
    ...
    public void union(PointsToGraph in) {
        ...
        for(String claves : in.mapping.keySet()){
            Set<Node> definicion = in.mapping.get(claves);
            if (this.mapping.containsKey(claves)) {
                definicion.addAll(this.mapping.get(claves));
            }
            this.mapping.put(claves, definicion);
        }
    }
}
